...
class HeadMotionAnalyzer:
    ...
    def __init__(self, window_size: int = 24, pitch_threshold: float = 9.0, yaw_threshold: float = 10.0, roll_threshold: float = 8.0) -> None:
        ...
        # This member stores the MediaPipe face mesh constructor namespace.
        self._mp_face_mesh = mp.solutions.face_mesh

    # This method computes lightweight 2D image landmarks from a frame.
    def _extract_landmarks(self, frame_bgr: np.ndarray, face_mesh: mp.solutions.face_mesh.FaceMesh) -> Optional[Tuple[np.ndarray, List[Tuple[float, float]]]]:
    ...
    def analyze_video(self, video_path: str) -> Dict[str, object]:
        ...
        frame_index = 0
        # This context manager initializes Face Mesh with refined landmarks.
        with self._mp_face_mesh.FaceMesh(static_image_mode=False, max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
            ...
            while True:
                ...
                extracted = self._extract_landmarks(frame, face_mesh)
